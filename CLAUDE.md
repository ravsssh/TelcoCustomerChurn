...
```
TelcoCustomerChurn/
├── AnalyticsAndModelling.ipynb   # EDA/research reference notebook — UNCHANGED
├── app.py                        # Streamlit home page (KPI cards, navigation)
├── train.py                      # CLI training script → saves all artifacts
├── src/
│   ├── __init__.py
│   ├── preprocessing.py          # encode_dataframe (saveable encoder), constants, inference helpers
│   ├── model.py                  # create_high_recall_model, find_optimal_threshold, get_callbacks
│   └── inference.py              # load_artifacts, predict_single, predict_batch, generate_recommendations
├── pages/
│   ├── 1_Train_Model.py          # UI: trigger training, live Keras log, post-training metrics
│   ├── 2_Model_Performance.py    # Confusion matrix, ROC, PR, feature importance, business impact
│   ├── 3_Single_Prediction.py    # Customer form → probability gauge + risk badge + recommendations
│   └── 4_Batch_Prediction.py     # CSV upload → predictions table + download
├── artifacts/                    # gitignored — generated at runtime by train.py
│   ├── churn_model.keras         # Trained Keras model (native format, not .h5)
│   ├── scaler.pkl                # Fitted StandardScaler
│   ├── encoder.pkl               # Fitted OneHotEncoder (critical: ensures column ordering for inference)
│   └── config.json               # threshold, metrics, curves, feature_names, training history
...
├── requirements.txt              # Pinned Python dependencies (includes streamlit==1.45.0)
...
> **Note**: `artifacts/` is git-ignored. Run `python train.py` to generate all artifacts locally
> before starting the Streamlit app. The notebook `AnalyticsAndModelling.ipynb` is kept unchanged
> as the EDA and research reference — it is NOT used during deployment.
...
# First time: train the model to generate artifacts/
python train.py
...
1. **Notebook is EDA-only**: `AnalyticsAndModelling.ipynb` is kept unchanged as the research artifact. All production logic lives in `src/` and `train.py`.
...
5. **No model artefacts in repo**: All files in `artifacts/` are git-ignored; treat them as local outputs generated by `train.py`.
...
7. **Encoder must be saved**: `encode_dataframe()` in `src/preprocessing.py` returns the fitted `OneHotEncoder`. Save it via `joblib.dump` at training time and load it for inference. Never call `fit_transform` on inference data.
8. **Feature count = 41**: `ServiceUsageScore` is computed for EDA only and is NOT fed to the model. `build_feature_matrix()` drops `customerID`, `Churn`, and `ServiceUsageScore`. This is confirmed by the SHAP shape `(100, 41, 1)` in the notebook.
9. **Keras format**: Use `.keras` (native SavedModel format), not `.h5`. The `.h5` format is deprecated in Keras 3.x.
10. **Streamlit cache pattern**: `load_artifacts()` in `src/inference.py` is a plain function (no Streamlit import). Each Streamlit page wraps it with `@st.cache_resource`. After retraining, call `st.cache_resource.clear()` to invalidate.
11. **Training runs as subprocess**: The Train Model page launches `train.py` via `subprocess.Popen` and streams stdout line-by-line. This prevents blocking the Streamlit event loop during long training runs.
12. **Visualisation style**: Call `setup_churn_style()` at the top of any new visualisation section in the notebook to maintain consistent plot aesthetics.
